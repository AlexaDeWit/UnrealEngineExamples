...
//Result will be returned in base64 encoding
...
FString UHmacSha256::ComputeBase64Signature(FString message, FString key)
{
  ...
  return UBase64Codec::Base64UrlEncode(result);
}
...
std::vector<UBYTE> UHmacSha256::ComputeSignature(FString message, FString key)
{
  ...
  {
    keyToUse.assign(keyString.begin(), keyString.end()); //Might be wrongly encoded for hashing
  }
